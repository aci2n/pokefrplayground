...
static void CB2_HandleStartBattle(void)
{
		...
		DebugPrintf("CB2_HandleStartBattle: %u", gBattleCommunication[MULTIUSE_STATE]);
...
static void BattleIntroGetMonsData(void)
{
		DebugPrintf("BattleIntroGetMonsData %u", gBattleCommunication[MULTIUSE_STATE]);
		switch (gBattleCommunication[MULTIUSE_STATE])
		{
		case 0:
				gActiveBattler = gBattleCommunication[1];
				DebugPrintf("active battler: %u", gActiveBattler);
...
static void BattleIntroDrawTrainersOrMonsSprites(void)
{
		...
		for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++) {
				DebugPrintf("BattleIntroDrawTrainersOrMonsSprites: active battler %u", gActiveBattler);
...
static void HandleTurnActionSelectionState(void)
{
		...
		for (gActiveBattler = 0; gActiveBattler < gBattlersCount; gActiveBattler++)
		{
				...
				DebugPrintf("determining action (battler %u): %u", gActiveBattler, gBattleCommunication[gActiveBattler]);
